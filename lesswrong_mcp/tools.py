...
async def lw_search(
    ...
    """
    # min_length=2 bounds the raw length, so all-whitespace slips through. The backend treats
    # a blank `search` as "no query" and serves its /api/search *documentation* page — a
    # silently-useless response — so reject it before the network call.
    if not query.strip():
        return "Error: empty search query. Pass at least one non-whitespace search term."
    # The shared index serves only the first SEARCH_RESULT_WINDOW results (page * limit <=
    # 10000). A deeper request is a deterministic 500 that would also burn the retry budget,
    ...
    notes: list[str] = []
    if content_type is not None:
        # With one type requested the page still labels its count "across all types" though
        # the number is that type's total. A no-op if the upstream wording changes.
        md = md.replace("- Results across all types:", f"- Results for {content_type.value}:", 1)
    else:
        # An all-types search fans `limit` across every type, so `limit=2` yields ~10 rows.
        notes.append(f"Showing up to {limit} results per content type (× {len(SearchType)} types).")
    # Only offer a next page this tool would actually accept: it must clear both bounds — the
    # page ceiling and the result window — or the hint would point at a page Pydantic rejects.
...
async def lw_filter_posts(
    ...
    # The date Fields only regex-check shape, which admits non-dates like '2025-02-30'. Parse
    # up front so date.fromisoformat can't raise an uncaught ValueError past _tool_errors.
    ...
    tag_ids: list[str] = []
    if tag:
        # Comma-separated tags require their intersection (tag="ai,interpretability"). Each
        # resolves independently and becomes its own Required filterSettings entry below.
        ...
        if not tag_ids:
            # `tag` was truthy but every segment blank (e.g. ","). Fail rather than silently
            # running unfiltered under a header that claims a tag filter.
            return f"Error: no tag found for {tag!r}. Pass a valid tag slug or tag _id."

    # Query the view named by the sort, applying any tag as a `filterSettings` required-tag —
    # the mechanism the site's own tag pages use. Every sort view honours it and it composes
    # with userId/after/before/karmaThreshold/af, so a tag combines cleanly with any sort.
    # (The dedicated tagRelevance view ties a tag to its own ordering, so it can't serve an
    # explicit sort; filterSettings keeps the two independent.)
    view = sort.value
    ...
    if before_date is not None and before_date < date.max:
        # The selector treats `before` as an exclusive midnight-start bound, so a raw date
        # would drop the boundary day. Advance one day to make it inclusive, like `after`.
    ...
    # Over-fetch one row so has_more needs no second round-trip; trimmed off below. (The
    # posts view accepts limit > 100, so limit+1 is safe even at the max limit.)
    ...
    # ForumMagnum no-ops karmaThreshold == 0 (it bounds baseScore only for non-zero
    # thresholds), so min_karma=0 would let negative-karma posts through. Enforce the
    # documented ">= min_karma" contract for every value; a no-op when the server filtered.
    if min_karma is not None:
        posts = [p for p in posts if (p["karma"] or 0) >= min_karma]

    if response_format == ResponseFormat.json:
        # These reflect the server's row count, computed before the min_karma safety net, so
        # they signal further *server* rows even when that filter trims this page.
        return json.dumps(
            {
                "count": len(posts),
                "has_more": has_more,
    ...
    # Mirror the paging signal in Markdown too, so it isn't JSON-only.
    if has_more:
    ...
    if not posts:
        # min_karma can empty this page while the server still has rows, so scope the lead to
        # "this page" rather than a flat — and, with a footer, contradictory — "nothing matched".
        lead = "No posts on this page matched those filters." if footer else "No posts matched those filters."
        return lead + footer

    base = _base(s)
    # Keep in lock-step with `inner` above: gated exactly as the selector is.
    crumb_specs = (
...
# Every tool is registered by now, so hoist each schema's nullable/enum constraints to the
# top level for schema-shallow clients (see server._apply_flat_schemas).
